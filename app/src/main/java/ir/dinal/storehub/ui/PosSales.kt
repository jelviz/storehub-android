...
fun PosScreen(activity: Activity, nav: NavHostController) {
    ...
    fun addProduct(p: ProductEntity) {
        ...
        if (i >= 0) {
            cart[i] = cart[i].copy(quantity = cart[i].quantity + 1)
        } else {
            cart.add(CartLine(p, 1.0))
        }
    }

    fun lookup(c: String) {
        scope.launch {
            store.findByCode(c)?.let {
                addProduct(it)
                code = ""
            } ?: run {
                msg = "کالا با این کد پیدا نشد."
            }
        }
    }
    ...
    Shell(nav, "صندوق فروش") {
        Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            Button({ if (code.isNotBlank()) lookup(code) }) {
                Text("افزودن")
            }
            OutlinedButton({
                scanner.startScan()
                    .addOnSuccessListener { barcode ->
                        barcode.rawValue?.let(::lookup)
                    }
                ...
                Card(Modifier.fillMaxWidth().padding(vertical = 3.dp)) {
                    Row(
                        ...
                        Column(Modifier.weight(1f)) {
                            ...
                            Text("${money(line.product.price)} × ${line.quantity}")
                        }
                        Row {
                            TextButton({
        ...
        Text(
            "جمع: ${money(cart.sumOf { it.product.price * it.quantity })}",
        ...
        OutlinedTextField(
            mobile,
            { mobile = it },
            label = { Text("موبایل مشتری (اختیاری)") },
            modifier = Modifier.fillMaxWidth()
        )
        ...
        Button(
            {
                scope.launch {
                    ...
                    msg = null
                    runCatching {
                        store.checkout(cart.toList(), payment, customer, mobile)
                    }.onSuccess { id ->
                        cart.clear()
                        customer = ""
                        mobile = ""
                        msg = "فروش ثبت شد. شماره داخلی: $id"
                    }.onFailure {
                        msg = it.message
                    }
                    busy = false
            ...
            items(list, key = { it.id }) { s ->
                Card(Modifier.fillMaxWidth().padding(vertical = 3.dp)) {
                    Column(Modifier.padding(10.dp)) {
                        ...
                        Text("مبلغ: ${money(s.total)} | مرجوعی: ${money(s.returnedTotal)}")
                        TextButton({
                            scope.launch {
                                details = store.saleDetails(s.id)
                                returnQty.clear()
                            }
                        }) {
                            Text("جزئیات / مرجوعی")
                ...
                LazyColumn(Modifier.heightIn(max = 450.dp)) {
                    items(d.items, key = { it.id }) { i ->
                        Column(Modifier.padding(vertical = 6.dp)) {
                            ...
                            Text(
                                "خرید: ${i.quantity} | مرجوع شده: ${i.returnedQuantity} | قابل مرجوعی: ${i.quantity - i.returnedQuantity}"
                            )
                            OutlinedTextField(
                                returnQty[i.id] ?: "",
                                { returnQty[i.id] = it },
                                label = { Text("تعداد مرجوعی") },
                                singleLine = true
                            )
                        }
                    }
            ...
            confirmButton = {
                Button({
                    scope.launch {
                        ...
                        runCatching {
                            store.returnSale(d.sale.id, q, "مرجوعی از اپ")
                        }.onSuccess {
                            details = null
                            load()
                        }.onFailure {
